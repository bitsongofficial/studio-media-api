...
export default defineEventHandler(async (event) => {
  ...
  try {
    const track = await prisma.$transaction(async (tx) => {
      const track = await tx.tracks.update({
        where: {
          id: _track.id,
          user_id: user.userId,
        },
      ...
      if (data.artists && data.artists?.length > 0) {
        await tx.track_artists.deleteMany({
          where: {
            track_id: _track.id,
          },
        })

        await tx.track_artists.createMany({
      ...
      if (data.authors_publishers && data.authors_publishers?.length > 0) {
        await tx.track_authors_publishers.deleteMany({
          where: {
            track_id: _track.id,
          },
        })

        await tx.track_authors_publishers.createMany({
      ...
      if (data.royalties_info && data.royalties_info?.length > 0) {
        await tx.track_royalties_info.deleteMany({
          where: {
            track_id: _track.id,
          },
        })

        await tx.track_royalties_info.createMany({
      ...
      if (data.marketplace) {
        await tx.track_marketplace.upsert({
          where: {
            id: _track.id,
          },
          create: {
